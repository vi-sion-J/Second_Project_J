...
class Matcher:
    def __init__(
        ...
        #기존에 뽑아둔 reference tensor를 메모리에 상주시킴 올렸다 내렸다 할일 없게
        self.anchor_benign = torch.load('/home/junha/New_Project/Matcher_mine/save_image/anchor_benign_centroid.pt').to(self.device)
        self.anchor_malignant = torch.load('/home/junha/New_Project/Matcher_mine/save_image/anchor_malignant_centroid.pt').to(self.device)
        #squeeze(0)를 통해 (1,1024) -> (1024) 순수한 1차원 벡터로 다듬음
        self.anchor_benign = self.anchor_benign.squeeze(0)
        self.anchor_malignant = self.anchor_malignant.squeeze(0)

    # 앵커를 이용해 benign, malignant 둘중 어떤걸 reference로 가져갈지 정함
    def route_target(self, tar_feat):
        
        #tar -> target image, b -> benign, m -> malignant
        # L2 정규화 F.normalize 벡터의 크기를 1로 통일 방향을 비교하기 위한 과정
        tar_global_norm = F.normalize(tar_feat, dim=0, p=2)
        anchor_b_norm = F.normalize(self.anchor_benign, dim=0, p=2)
        anchor_m_norm = F.normalize(self.anchor_malignant, dim=0, p=2)

        # target 이미지의 모든 패치(N개)와 앵커 간의 코사인 유사도를 행렬 곱으로 일괄 계산 -> 결과 차원: [N]
        sim_b_patches = torch.matmul(tar_global_norm, anchor_b_norm)
        sim_m_patches = torch.matmul(tar_global_norm, anchor_m_norm)

        # 노이즈 배경을 버리고 가장 반응이 강한 상위 K(50)개 패치만 선별하여 평균 계산
        k = min(30, sim_b_patches.size(0))
        sim_benign = sim_b_patches.topk(k)[0].mean()
        sim_malignant = sim_m_patches.topk(k)[0].mean()


        # 더 유사한 앵커 반환
        if sim_benign > sim_malignant:
            self.benign_count += 1
            print(f"[Top-K 라우팅] 양성: {sim_benign:.4f} | 악성: {sim_malignant:.4f} -> 판정: 양성 (누적 양성: {self.benign_count}, 누적 악성: {self.malignant_count})")
            return self.anchor_benign, "benign"
        else:
            self.malignant_count += 1
            print(f"[Top-K 라우팅] 양성: {sim_benign:.4f} | 악성: {sim_malignant:.4f} -> 판정: 악성 (누적 양성: {self.benign_count}, 누적 악성: {self.malignant_count})")
            return self.anchor_malignant, "malignant"
    ...
    def predict(self):
        ...
        # route_target 함수 연결: 타겟이 양성인지 악성인지 판별
        # 기존의 픽셀 매칭 로직으로 넘어가기전에 라우팅 함수를 넣어서 흐름 바꾸기 route_target에서 클래스에 해당하는 tensor와, 클래스를 순서대로 뽑음
        best_anchor, pred_class = self.route_target(tar_feat)
